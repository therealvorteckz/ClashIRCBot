    ...
    async def handle(self, data: str):
        try:
            ...
            if args[0] == 'PING':
                    ...
                    await self.raw('NICK ' + self.nickname + '_') # Send the new nickname to the server
            elif args[1] == 'KICK':
                    chan   = args[2]
                    kicked = args[3]
                    ...
                    if target.startswith(''): # Channel message
                        if msg.startswith('!'):
                            if target == config.irc.channel or target == self.nickname or target == self.nickname + '_':
                                try:    
                                    if time.time() - config.throttle.last < config.throttle.cmd and config.throttle.lastnick == nick:
                                        ...
                                        if arguments[0] == '!hug':
                                            ...
                                            if clan.public_war_log == True:
                                                ...
                                                await bot.sendmsg(config.irc.channel, f'{color("[", colors.color1)}{color("Wars Won:", colors.color2)} {color(clan.war_wins, colors.color3)}{color("]", colors.color1)}')
